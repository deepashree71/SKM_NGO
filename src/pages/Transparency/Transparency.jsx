...
export default function Transparency() {
  ...
  const handleDownload = async (doc) => {
    ...
    try {
      ...
      toast.success(`Downloaded: ${doc.title}`, { id: toastId })
    } catch (err) {
      // Fallback: if API not available (dev/demo), show friendly message
      const status = err?.response?.status
      if (status === 404) {
        toast.error('Document not found on server.', { id: toastId })
      } else if (status === 401 || status === 403) {
        toast.error('Please log in to download documents.', { id: toastId })
      } else {
        toast.error('Download failed. Please try again later.', { id: toastId })
      }
    } finally {
      setDownloading(null)
    }
  ...
  const handlePreview = async (doc) => {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        toast.error('Please log in to view documents.', { id: toastId })
      } else {
        toast.error('Could not open document. Please try again.', { id: toastId })
      }
    } finally {
      setPreviewing(null)
    }
              ...
              onMouseOut={e => { e.currentTarget.style.borderColor='#e8ecf4'; e.currentTarget.style.boxShadow='0 2px 8px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                  <button
                    ...
                    style={{ width: 36, height: 36, borderRadius: 6, border: '1px solid #e8ecf4', background: '#F7F8FA', cursor: previewing === doc.id ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: previewing === doc.id ? 0.6 : 1 }}>
                    {previewing === doc.id
                      ? <Loader size={16} color="#666" style={{ animation: 'spin 1s linear infinite' }} />
                  ...
                  <button
                    ...
                    style={{ padding: '7px 14px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 5, opacity: downloading === doc.id ? 0.6 : 1, cursor: downloading === doc.id ? 'not-allowed' : 'pointer' }}>
                    {downloading === doc.id
