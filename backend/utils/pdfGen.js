/**
 * pdfGen.js — Vercel-compatible PDF generation
 *
 * Uses puppeteer-core + @sparticuz/chromium so it works on Vercel's
 * serverless environment (no full Chromium install needed).
 *
 * On local dev it falls back to a regular puppeteer launch if
...
const launchBrowser = async () => {
  ...
  return puppeteer.launch({
    ...
    defaultViewport: getChromium().defaultViewport,
    executablePath,
    headless: getChromium().headless ?? true,
  })
}
...
const generateCertificatePDF = async (cert) => {
  ...
  <style>
    ...
    .recipient { font-size: 42px; color: #2c3e50; margin: 16px 0; font-style: italic; }
